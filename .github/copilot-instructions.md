# GitHub Copilot Instructions for Renalinfolk Theme
...
1. **Context Awareness**: Always check which file type you're working with (HTML templates, PHP patterns, JSON config)
2. **Block Syntax**: Use proper WordPress block comment syntax when suggesting block markup
3. **Validation**: Suggest validation commands after making changes to theme.json or style variations
4. **Namespace**: Always use `renalinfolk` prefix for functions, patterns, and text domains
5. **Color References**: Use CSS custom properties instead of hardcoded color values
6. **Accessibility**: Consider WCAG AA compliance when suggesting color combinations
7. **Documentation**: Reference official WordPress Block Editor documentation for block-related suggestions
